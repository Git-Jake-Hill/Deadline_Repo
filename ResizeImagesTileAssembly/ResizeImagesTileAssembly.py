...
## Custom edits starting at line 344 - 428
##############################################################################################
...
class DraftEventListener (DeadlineEventListener):
	...
	def OnJobSubmitted(self, job):
		...
		print("Looking for Tile Job...")
		
		tile_assembley = "Draft Assembler Dependent Job Submission from 3ds Max"
		if job.GetJobInfoKeyValue("Comment") == tile_assembley:
			print("Found Tile Job.")

			
			print("-----Start [RESIZE] draft job-----")
			max_parent_jobId = job.JobDependencyIDs
			max_parent_job = RepositoryUtils.GetJob(max_parent_jobId[0], True)
			print(" ***Parent job name?:", max_parent_job.JobName)
			...
			# job.JobName, ms_tag = original_name.rsplit( '[EXr-TRACT]', 1 ) # not needed for tile assembly

			
			# access the values for Final Image Size
			FinalImageWidth = max_parent_job.GetJobExtraInfoKeyValue("FinalImageWidth") 
			FinalImageHeight = max_parent_job.GetJobExtraInfoKeyValue("FinalImageHeight") 
			print(" ****Parent image width x height:", FinalImageWidth, FinalImageHeight)

			# set dependency of new job on current job
			exrtract_job = job.JobId
