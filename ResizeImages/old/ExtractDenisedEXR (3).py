...
class DraftEventListener (DeadlineEventListener):
	...
	def OnJobFinished(self, job):
		...
		if job.JobName.endswith("[MAXSCRIPT]") and job.JobName.find("DENOISE") != -1:
			print("Start draft job")
			self.LogInfo(job.JobOutputFileNames[0])
			image, exten = job.JobOutputFileNames[0].rsplit( '.', 1 )
			outFile = job.JobOutputDirectories[0] + "\\" + image + "_denoised." + exten # TODO remove test file
			self.LogInfo(outFile)
			draftTemplate_jh = RepositoryUtils.GetRepositoryFilePath("events/ExtractDenisedEXR/split_exr_channels_single_image_Draft.py", True)
			
			print("Name =", job.JobName)
			
			layerList = job.GetJobExtraInfoKeyValue("REToDenoise") # this works to access the values for a job script
			...
			scriptArgs = []
			scriptArgs.append('layers="%s" ' % layerList)
			...
			# self.CreateDraftJob(draftQuickScript, job, "Quick Draft", outputIndex=0, draftArgs=scriptArgs,mode=None, quickDraftFormat=format)

			# ScriptArg8=inFile="R:\Project\BMU006 Queens Wharf - IRD + T5 & 6\Images\V98\V98.0000.exr" 
			# scriptFile=G:\Temp\JH\Scripts\Deadline\Split multi-channel EXR\split_exr_channels_single_image.py

			try:
				self.CreateDraftJob(draftTemplate_jh, job, "EXr-TRACT",0, outFileNameOverride=outFile, draftArgs=scriptArgs)
				self.LogInfo("Draft job submit success")
			
			except:
				self.LogInfo("Failed to create draft job")
