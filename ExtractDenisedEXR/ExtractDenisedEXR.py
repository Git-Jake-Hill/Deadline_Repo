...
class DraftEventListener (DeadlineEventListener):
	...
	def OnJobSubmitted(self, job):
		...
		if job.JobName.endswith("[MAXSCRIPT]") and job.JobName.find("DENOISE") != -1:
			...
			# load template python file that will actulaly extract the layers
			...
			# change name to remove [MAXSCRIPT] tag from draft job in deadline
			print("Name:", job.JobName)
			original_name = job.JobName
			job.JobName, ms_tag = original_name.rsplit( '[MAXSCRIPT]', 1 )
			...
			self.LogInfo(" ")
			print("Image width x height:", FinalImageWidth, FinalImageHeight)
			job.SetJobExtraInfoKeyValue("FinalImageWidth", FinalImageWidth)
			job.SetJobExtraInfoKeyValue("FinalImageHeight", FinalImageHeight)
			
			# job.JobExtraInfo1("test value string")

			# set dependency of new job on current job
			parent_job = job.JobId
			...
			scriptArgs = []
			# pass list of layers that will be extracted
			scriptArgs.append('REToDenoise="%s" ' % layerList)
			...
			# -------- finsih of [EXr-TRACT] script here --------
